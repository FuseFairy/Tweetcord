...
if not check_env():
    log.critical(
        "incomplete environment variables detected, please check your .env file. Exiting..."
    )
    sys.exit(1)

if not build_and_validate_configs():
    log.critical(
        "failed to build application configuration, please check logs for details. Exiting..."
    )
    sys.exit(1)
...
async def on_ready():
    if not (
        os.path.isfile(os.path.join(os.getenv("DATA_PATH"), "tracked_accounts.db"))
    ):
        await init_db()
    ...
    if invalid_clients:
        ...
        if configs["auto_repair_mismatched_clients"]:
            ...
            log.info(
                "automatically replace mismatched client names with the first client name in the environment variable, use the sync slash command in discord to ensure notifications are turned on"
            )
        else:
            log.warning(
                "set auto_repair_mismatched_clients to true in configs to automatically fix this error or manually update the database or environment variables"
            )
    else:
...
async def download_data(ctx: commands.context.Context):
    message = await ctx.send(
        file=discord.File(os.path.join(os.getenv("DATA_PATH"), "tracked_accounts.db"))
    )
    await message.delete(delay=15)
...
async def on_tree_error(itn: discord.Interaction, error: app_commands.AppCommandError):
    ...
    log.warning(
        f"an error occurred but was handled by the tree error handler, error message : {error}"
    )


@bot.event
async def on_command_error(
    ctx: commands.context.Context, error: commands.errors.CommandError
):
    ...
    log.warning(
        f"an error occurred but was handled by the command error handler, error message : {error}"
    )
