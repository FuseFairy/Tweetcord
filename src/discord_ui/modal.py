...
class CustomizeMsgModal(discord.ui.Modal):
    def __init__(
        self, user_id: str, username: str, channel: discord.TextChannel | discord.Thread
    ):
        super().__init__(title=t("modal.customize_message.title"), timeout=None)
        self.user_id = user_id
        self.channel = channel
        label = t(
            "modal.customize_message.label_full",
            username=username,
        ...
        self.customized_msg = discord.ui.TextInput(
            label=label,
            placeholder=t("modal.customize_message.placeholder"),
            max_length=200,
        ...
        async with aiosqlite.connect(
            ...
            async with lock:
                await db.execute(
                    ...
                    (self.customized_msg.value, self.user_id, str(self.channel.id)),
                )
                await db.commit()
