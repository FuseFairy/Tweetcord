...
async def init_db():
    ...
    async with aiosqlite.connect(
        ...
        await db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS notification (user_id TEXT, channel_id TEXT, role_id TEXT, enabled INTEGER DEFAULT 1, enable_type TEXT DEFAULT 11, enable_media_type TEXT DEFAULT 11, customized_msg TEXT DEFAULT NULL, qq_group_id TEXT DEFAULT NULL, FOREIGN KEY (user_id) REFERENCES user (id), FOREIGN KEY (channel_id) REFERENCES channel (id), PRIMARY KEY(user_id, channel_id));
        ...
        """)
        # Migration: Add qq_group_id if it doesn't exist
        async with db.execute("PRAGMA table_info(notification)") as cursor:
            columns = [row[1] for row in await cursor.fetchall()]
            if "qq_group_id" not in columns:
                await db.execute(
                    "ALTER TABLE notification ADD COLUMN qq_group_id TEXT DEFAULT NULL"
                )
        await db.commit()
