...
class AccountTracker:
    ...
    async def setup_tasks(self):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        log.error(
                            f"persistent authentication failure for account {account_name}"
                        )
                        raise
        ...
        for account_name, account_token in self.accounts_data.items():
            try:
                ...
                self.bot.loop.create_task(self.tweetsUpdater(app)).set_name(
                    f"TweetsUpdater_{account_name}"
                )
            except Exception:
                sys.exit(1)
            ...
            try:
                async with connect_readonly(self.db_path) as db:
                    async with db.execute(
                        "SELECT username, client_used, latest_tweet FROM user WHERE enabled = 1"
                    ) as cursor:
                        new_timestamps = {}
    ...
    async def notification(self, username: str, client_used: str):
        while True:
            ...
            latest_tweets = await get_tweets(
                self.tweets[client_used], username, last_tweet_at
            )
            ...
            self.latest_tweet_timestamps[(username, client_used)] = str(
                newest_timestamp
            )
            ...
            try:
                    ...
                    async with db.cursor() as cursor:
                        ...
                        if user:
                            if EMBED_TYPE == "proxy" and AUTO_TRANSLATION["enabled"]:
                                ...
                                await cursor.execute(
                                    "SELECT * FROM notification WHERE user_id = ? AND enabled = 1",
                                    (user["id"],),
                                )
                ...
                if "database is locked" in str(e):
                    log.warning(
                        f"database locked while reading notification settings for {username}, this is unexpected but handled."
                    )
                else:
                    raise
            ...
            for tweet in latest_tweets:
                ...
                if (
                    bool(tweet.media)
                    and tweet.media[0].type == "video"
                    and EMBED_TYPE == "built_in"
                    and configs["embed"]["built_in"]["video_link_button"]
                ):
                    create_view = True
                    button_label, button_url = (
                        t("display.button.view_video"),
                        tweet.media[0].expanded_url,
                    )
                elif (
                    EMBED_TYPE == "proxy"
                    and configs["embed"]["proxy"]["original_url_button"]
                ):
                    create_view = True
                    button_label, button_url = (
                        t("display.button.view_original"),
                        tweet.url,
                    )
                ...
                for data in notifications:
                    ...
                    if (
                        channel is not None
                        and is_match_type(tweet, data["enable_type"])
                        ...
                        try:
                            ...
                            if EMBED_TYPE == "proxy":
                                ...
                                if AUTO_TRANSLATION["enabled"]:
                                    lang = (
                                        data["server_translate"]
                                        if data["server_translate"] is not None
                                        else AUTO_TRANSLATION["default_language"]
                                    )
                                    url += f"/{lang}"
                            ...
                            if not data["customized_msg"]:
                                ...
                                msg = (
                                    re.sub(
                                        r":(\w+):",
                                        lambda match: replace_emoji(
                                            match, channel.guild
                                        ),
                                        data["customized_msg"],
                                    )
                                    if configs["emoji_auto_format"]
                                    else data["customized_msg"]
                                )
                            msg = msg.format(
                                mention=mention, author=author, action=action, url=url
                            )
                            ...
                            if EMBED_TYPE == "proxy":
                                ...
                                footer = (
                                    "twitter.png"
                                    if configs["embed"]["built_in"]["legacy_logo"]
                                    else "x.png"
                                )
                                file = discord.File(
                                    f"images/{footer}", filename="footer.png"
                                )
                                await channel.send(
                                    msg,
                                    file=file,
                            ...
                            if not isinstance(e, discord.errors.Forbidden):
                                log.error(
                                    f"an error occurred at {channel.mention} while sending notification: {e}"
                                )
    ...
    async def tweetsUpdater(self, app: Twitter):
        ...
        while True:
            try:
                ...
                self.tweets[updater_name] = await asyncio.to_thread(
                    app.get_tweet_notifications
                )
            except KeyError as e:
                # Handle the error thrown by `tweety-ns` mentioned in issue#59
                log.warning(
                    f"handled KeyError in {updater_name}: {e}. This is likely a temporary API response issue from Twitter. Skipping this check."
                )
            except Exception as e:
                log.error(f"{e} (task : tweets updater {updater_name})")
                log.error(
                    f"an unexpected error occurred, try again in {configs['tweets_updater_retry_delay']} minutes"
                )
    ...
    async def tasksMonitor(self):
        ...
        while True:
            ...
            users_in_cache = {
                username for username, _ in self.latest_tweet_timestamps.keys()
            }
            ...
            if alive_tasks != users_in_cache:
                ...
                if dead_tasks:
                    ...
                    for dead_task_username in dead_tasks:
                        ...
                        if client_used:
                            self.bot.loop.create_task(
                                self.notification(dead_task_username, client_used)
                            ).set_name(dead_task_username)
                            log.info(
                                f"restart {dead_task_username} successfully using {client_used}"
                            )
            ...
            if (
                datetime.now(timezone.utc) - self.tasksMonitorLogAt
            ).total_seconds() / 3600 >= configs["tasks_monitor_log_period"]:
    ...
    async def addTask(self, username: str, client_used: str):
        ...
        # Start the task
        self.bot.loop.create_task(self.notification(username, client_used)).set_name(
            username
        )
