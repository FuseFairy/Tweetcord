...
async def gen_embed(tweet: Tweet) -> list[discord.Embed]:
    ...
    embed.set_thumbnail(
        url=re.sub(r"normal(?=\.jpg$)", "400x400", tweet.author.profile_image_url_https)
    )
    embed.set_footer(
        text="Twitter" if configs["embed"]["built_in"]["legacy_logo"] else "X",
    ...
    if len(tweet.media) == 1:
        ...
        if configs["embed"]["built_in"]["fx_image"]:
            ...
            fximage_url = BeautifulSoup(raw, "html.parser").find(
            ...
            imgs_embed = [
                discord.Embed(url=tweet.url).set_image(url=media.media_url_https)
